...
class NetworkSession{
	...
	/**
	 * The original protocol reported by the client during the handshake (RequestNetworkSettingsPacket).
	 * Used for multi-version support: other code can check this value to
	 * decide how to encode/decode packets or map blocks/items for this session.
	 * Defaults to ProtocolInfo::CURRENT_PROTOCOL until the handshake actually completes.
	 */
	...
	/**
	 * The original protocol reported by this client (see the $protocolVersion property).
	 */
	public function getProtocolVersion() : int{ return $this->protocolVersion; }

	/**
	 * @internal Only called by SessionStartPacketHandler once the handshake succeeds.
	 */
	...
	public function startUsingChunk(int $chunkX, int $chunkZ, \Closure $onCompletion) : void{
		...
		$legacyProtocolVersion = LegacyPacketRegistry::isLegacySession($this->protocolVersion) ? $this->protocolVersion : 0;
		$promiseOrPacket = ChunkCache::getInstance($world, $this->compressor, $legacyProtocolVersion)->request($chunkX, $chunkZ);
