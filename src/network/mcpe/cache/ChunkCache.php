...
class ChunkCache implements ChunkListener{
	 ...
	 *
	 * MultiVersion: $legacyProtocolVersion (0 = modern/default) adds an extra cache dimension so that
	 * legacy-protocol sessions (each with their own different block palette) get their own separately
	 * cached/serialized chunk data, instead of sharing the modern-protocol cache (which would have the
	 * wrong block runtime IDs for them).
	 */
	public static function getInstance(World $world, Compressor $compressor, int $legacyProtocolVersion = 0) : self{
		$worldId = spl_object_id($world);
		$compressorId = spl_object_id($compressor);
		$variant = $legacyProtocolVersion;
		...
		if(!isset(self::$instances[$worldId][$compressorId][$variant])){
			\GlobalLogger::get()->debug("Created new chunk packet cache (world#$worldId, compressor#$compressorId, legacyProtocolVersion=$legacyProtocolVersion)");
			self::$instances[$worldId][$compressorId][$variant] = new self($world, $compressor, $legacyProtocolVersion);
		}
		return self::$instances[$worldId][$compressorId][$variant];
	}

	public static function pruneCaches() : void{
		foreach(self::$instances as $compressorMap){
			foreach($compressorMap as $variantMap){
				foreach($variantMap as $chunkCache){
	...
	private function __construct(
		...
		private int $legacyProtocolVersion = 0,
